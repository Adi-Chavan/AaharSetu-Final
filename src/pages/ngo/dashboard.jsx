...
export function NgoDashboard() {
  ...
  const [ngoRequestedDonations, setNgoRequestedDonations] = useState([]); // NGO requested donations
  ...
  const handleRequestSubmit = async (e) => {
    ...
    try {
      ...
      body: JSON.stringify( {title: requestForm.title,
      ...
      <div className="max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-8 flex justify-between items-center">
          <div>
            ...
            <p className="text-neutral-600">Review donations, request new ones, and manage approvals</p>
          </div>

          <div className='flex space-x-4'>
            <Link to="/ml">
              <Button>ML Recommendations</Button>
            </Link>
            ...
            {approvedDonations.length > 0 ? (
              <div className="space-y-4">
                {approvedDonations.map((donation) => (
                  <div key={donation._id} className="bg-white/50 p-4 rounded-lg">
                    <h3 className="font-medium">{donation.title}</h3>
                    <p className="text-blue-600 font-semibold text-sm">{donation.claimedBy ? "Assigned to a Volunteer" : "Awaiting Volunteer"}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-4">
                {ngoRequestedDonations.map((request) => (
                  <div key={request._id} className="bg-white/50 p-4 rounded-lg">
                    ...
                    <p className="text-neutral-600">{request.quantity} servings</p>
                  </div>
                ))}
              </div>
...
{showRequestForm && (
  <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
    <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="bg-white rounded-2xl p-6 max-w-md w-full">
      <h3 className="text-xl font-semibold mb-4">Request Donation</h3>
      <form onSubmit={handleRequestSubmit} className="space-y-4">
        
        <div>
          <label className="block text-sm font-medium mb-1">Title*</label>
          <input type="text" required className="w-full px-3 py-2 rounded-lg border border-neutral-200 bg-white/50"
        ...
        <div>
          ...
          <textarea required className="w-full px-3 py-2 rounded-lg border border-neutral-200 bg-white/50 h-24"
        ...
        <div>
          ...
          <input type="number" required min="1" className="w-full px-3 py-2 rounded-lg border border-neutral-200 bg-white/50"
        ...
        <div>
          ...
          <input type="date" required className="w-full px-3 py-2 rounded-lg border border-neutral-200 bg-white/50"
            value={requestForm.requiredBy}
            onChange={(e) => setRequestForm({ ...requestForm, requiredBy: e.target.value })} />
        </div>

        <div className="flex gap-3 pt-4">
          <Button type="submit" className="flex-1" onClick={handleRequestSubmit}>
            Submit Request
          </Button>
