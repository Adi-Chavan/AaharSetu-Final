...
//     if (!formData.address) newErrors.address = 'Address is required';
//     if (formData.areasServed.length === 0) newErrors.areasServed = 'Please select at least one area';
...
//     if (!formData.fssaiCompliance) newErrors.fssaiCompliance = 'FSSAI compliance document is required';
//     if (formData.pickupTiming.length === 0) newErrors.pickupTiming = 'Please select at least one pickup timing';
...
//         navigate('/ngo/dashboard');
//       } else {
//         setErrors({ submit: data.error || 'Failed to register NGO. Try again.' });
//       }
//     } catch (error) {
...
//   //   try {
//   //     const { termsAccepted, ...ngoData } = formData;
//   //     addNGO(ngoData);
//   //     navigate('/ngo/dashboard');
//   //   } catch (error) {
//   //     console.error('Error submitting NGO registration:', error);
...
//         setErrors({ ...errors, [field]: 'Please upload a PDF file' });
//         return;
//       }
//       if (file.size > 5 * 1024 * 1024) {
//         setErrors({ ...errors, [field]: 'File size should be less than 5MB' });
//         return;
//       }
...
//               <label className="block text-sm font-medium mb-2">Dietary Preferences</label>
//               <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
//                 {['Vegetarian','Non-Vegetarian', 'No Restrictions'].map((pref) => (
//                   <label key={pref} className="flex items-center space-x-2">
//                     <input
//                       type="checkbox"
...
//                         setFormData(prev => ({ ...prev, dietaryPreferences: updatedPrefs }));
//                       }}
//                       className="rounded border-neutral-300"
//                     />
//                     <span className="text-sm">{pref}</span>
